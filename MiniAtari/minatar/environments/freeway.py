...
# after 2500 frames have elapsed. (-1 reward if hit a car)
#
#####################################################################################################################
class Env:
    ...
    def act(self, a):
        ...
        elif(a=='n' and self.move_timer==0):
            self.move_timer = player_speed
        ...
        if(self.terminate_timer < 0 or has_won or is_crashed):
            self.terminal = True
        return r, self.terminal
    ...
    def game_state(self):
        return self.pos, copy.deepcopy(self.cars), self.move_timer
    ...
    def transition_function(self, state, action):
        ...
        tmp_env.reset()
        tmp_env.pos = state[0]
        tmp_env.cars = state[1]
        tmp_env.move_timer = state[2]
