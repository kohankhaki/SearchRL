...
class Env:
    ...
    def act(self, a, is_corrupted=False):
        ...
        if(self.alien_move_timer==0):
            self.alien_move_timer = min(np.count_nonzero(self.alien_map),self.enemy_move_interval) # change this to make game easier
    ...
    def _nearest_alien(self, pos):
        ...
        print(self.alien_map, self.terminal)
        return None
    ...
    def game_state(self):
        return self.pos, copy.deepcopy(self.f_bullet_map), copy.deepcopy(self.e_bullet_map), copy.deepcopy(self.alien_map), \
                self.alien_dir, self.enemy_move_interval, self.alien_move_timer, self.alien_shot_timer, \
                self.shot_timer
    ...
    def set_state(self, state):
        self.pos = state[0]
        self.f_bullet_map = copy.deepcopy(state[1])
        self.e_bullet_map = copy.deepcopy(state[2])
        self.alien_map = copy.deepcopy(state[3])
        self.alien_dir = state[4]
        self.enemy_move_interval = state[5]
        self.alien_move_timer = state[6]
